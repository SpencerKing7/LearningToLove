...
currently serves `www.spencerking7.com` from the `gh-pages` branch via the `gh-pages` npm package.

## What's already known

- Current setup: `homepage` in `package.json` points at the domain, `public/CNAME` holds
  `www.spencerking7.com`, deploys push a build to the `gh-pages` branch.
- Migration steps, in order:
  ...
  2. Add `www.spencerking7.com` as the project's domain and update DNS at the registrar to
